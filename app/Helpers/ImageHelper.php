...
class ImageHelper
{
    public static function uploadImage($image, $imageCat)
    {
        if (is_array($image)) {
            $imagePaths = [];

            foreach ($image as $singleImage) {
                $imagePaths[] = self::uploadSingleImage($singleImage, $imageCat);
            }

            return json_encode($imagePaths);
        } else {
            return self::uploadSingleImage($image, $imageCat);
        }
    }

    private static function uploadSingleImage($image, $imageCat)
    {
        $imageName = self::generateEncryptedName($image);

        $datePath = now()->format('Y/m/d');
        $filePath = "uploads/{$imageCat}/{$datePath}";
