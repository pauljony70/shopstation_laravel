        ...
        <div class="modal-dialog  modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-body p-4">
                    <form id="addForm" class="" action="{{ route('admin.brands.store') }}" method="post"
                        ...
                        <div class="form-group">
                            ...
                            <select id="brand_status" class="form-control" name="status">
                        ...
                        <div class="card">
                            <div class="card-body">
                                <button type="button" class="btn btn-info waves-effect waves-light mb-lg-3"
                                    data-toggle="modal" data-target=".add-btn">Add New Brand</button>
                                                    ...
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton"
                                                            ...
                                                            <a href="#" class="dropdown-item edit-btn"
                                                                data-toggle="modal" data-id="{{ $brand->id }}"><i
                                                ...
                                                <td>
                                                    <div class="">
                                                        <img src="{{ asset('storage/' . $brand->brand_img) }}"
                                                            alt="table-user" class="mr-2 align-middle" height="75">
                                                        <span class="align-middle"
                                                            style="display: inline-block; white-space: break-spaces; break-spaces; max-width:150px;">{{ $brand->brand_name }}</span>
                                                    </div>
                                                </td>
                                                <td>
                                                    @if ($brand->status == 1)
        ...
        $(document).ready(function() {
            ...
            window.Parsley.addValidator('addformname', {
                validateString: function(value) {
                    // console.log(value);
                    return $.ajax({
                        url: "{{ route('admin.brands.unique-name') }}",
                        method: "POST",
                        data: {
                            name: value,
                        },
            ...
            $('body').on('click', '.edit-btn', function() {
                ...
                $.get("{{ route('admin.brands.edit', '') }}/" + id, function(data) {
                    $.busyLoadFull("hide");
                    $('#edit-modal').modal('show');
                    $("#editForm").attr("action", "{{ route('admin.brands.update', '') }}/" + data
                        .id);
                    $('#editForm #name').val(data.brand_name);
                    var imagenUrl = "{{ asset('storage') }}/" + data.brand_img;
                    var drEvent = $('#editForm #image').dropify();
                    drEvent = drEvent.data('dropify');
                    drEvent.settings.defaultFile = imagenUrl;
                    drEvent.destroy();
                    drEvent.init();
                    $('#editForm #brand_status option[value="' + data.status + '"]').prop(
                        'selected', true);
                });
            });
                ...
                Swal.fire({
                    ...
                    .then((res) => {
                        if (res.value) {
                            $.ajax({
                                type: 'DELETE',
                                url: "{{ route('admin.brands.destroy', ['id' => '__id__']) }}"
                                    .replace('__id__', id),
